...
module PSRAM_READER (
    ...
    wire [7:0]  FINAL_COUNT = 2 + 13 + size*2; // was 22(spi:27): Always read 1 word
    ...
    wire[1:0] byte_index = {counter[7:1] - 8'd8}[1:0];
    always @ (posedge clk)
        if(counter >= 16 && counter <= FINAL_COUNT)
            if(sck)
                data[byte_index] <= {data[byte_index][3:0], din}; // Optimize!

    assign dout     =   (counter == 0)  ?   4'b0000             :
                        (counter == 1)  ?   4'b0001             :
...
// Using 38H Command, first bit is 1 if in QPI mold
module PSRAM_WRITER (
    input   wire            clk,
    input   wire            rst_n,
    input   wire [23:0]     addr,
    ...
    wire[7:0]        FINAL_COUNT = 2 + 7 + size*2;
